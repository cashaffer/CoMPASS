...
$idclass = mysql_real_escape_string($_GET['Class']);
...
$db2 = mysql_connect("144.92.160.26", "root", "cao2");
...
while($db->next_record()){
	...
	if ($navdate == $selecteddate) {
		//echo "matched on date";
		
		// use this for whole class graph, get logdata
		//echo $db->Record['idexploration'] . " ";
		$Requete = "SELECT * FROM logdata WHERE idexploration = " . $db->Record['idexploration'] ;
		$Result  = mysql_query($Requete,$db2);
		...
		//echo "found row";
		  ...
		  if ($currenttopicconcept == "\n()") {
			...
			$Result4  = mysql_query($Requete4,$db2);
...
$Requete6 = "SELECT name FROM class WHERE idclass = " . $idclass;
  $Result6  = mysql_query($Requete6,$db2);
...
, "R"=>0, "G"=>0, "B"=>0);
$title="CoMPASS Navigation - Class ".$classname . " - " . $selecteddate;
