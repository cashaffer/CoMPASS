	...
	if(!$priority->checkPage(8))
		...
		if($iduser != null){
			foreach ($iduser as $uid){	
				$sql = "insert into GROUPMEMBERS(idgroup,idstudent) values("
					.$idgroup.",".$uid.")";			
				$query = $db->query($sql);
			}
		}
		else
			header("location:studentlist1.php?cid=".$idgroup); 
...
<table width=528 cellspacing=1 cellpadding=4 border=0 align=center class=bgcolor5>
	...
	<td colspan=2 height=150 align=center class=bgcolor1> <p>
        <?= "Students are added successfully to the Group!<br> Click <a href='studentlist1.php?gid=".$idgroup."'>here</a> to continue"?>
        <br>
      </p>
	</td>
	</tr>
