...
function insert_concept($c, $t, $tim) {
	...
	global $total_time_cid, $total_time_tid;
	global $debug, $moves, $last_topic, $last_concept;
...
//	$explorationid = $_REQUEST['id'];
	if ($explorationid == null) {
		echo "You are currently logged out of the system. Please <a href='http://compassproject.net'>sign in</a> to get suggestions.";
		exit;
	} else {
		...
		$max_clicks = 40;
		if ($num_rows_in_result >= $max_clicks) {
			// too many results. take only the last 30.
			for ($i=$num_rows_in_result; $i>=$max_clicks; $i--) {
...
function get_num_visited_in_topic($t) {
		global $cids, $tids, $times, $cnum, $concept_topics_map, $topic_time_map, $concept_time_map, $random_browsing, $unread_concepts, $cluster_weight;
	global $total_time_cid, $total_time_tid;
	...
	$visited = 0;
	for ($i=90; $i<=140; $i++) {
		if (isset($concept_topics_map["".$i]) && isset($concept_topics_map["".$i]["".$t])) {
			$visited++;
		}
	}
	return $visited;
...
//	html_visited_all_concepts();
	...
	for ($i=1; $i<$cnum; $i++) {
		...
		if (!$tt) $tt = 'nil';
		if (!$tc) $tc = 'nil';
		$moves .= "(move (from-topic $ft) (from-concept $fc) (to-topic $tt) (to-concept $tc) (delta-time $tim) (timestamp $i)) \n";
	}
	...
	/* detect if goal topic is being visited */
	$num_visited_in_goal_topic = get_num_visited_in_topic($goal_tid);
	$total_concepts_in_goal_topic = 11; // for topics in simple machines
	$percent_visited = round($num_visited_in_goal_topic*100.0/(1.0*$total_concepts_in_goal_topic));

	/* detect random browsing */
	$random_browsing = 0;
	if ($cnum >= 3) {
//		echo 		$tids[$cnum - 3] . " " . $tids[$cnum - 2] ;
		if ( ($tids[$cnum - 2] != $tids[$cnum - 3]) &&  // Note the latest click would not have reached database yet.
			($tids[$cnum - 3] != $tids[$cnum - 4]) &&  // hence use information from earlier clicks.
			($tids[$cnum - 4] != $tids[$cnum - 2]) ) {
				$random_browsing = 1;
			}
	}

	/* detect the last time goal topic was visited */
	$goal_last_visited = 0;
	for ($i=0; $i<$cnum; $i++) {
		if ($tids[$i] == $goal_tid) {
			$goal_last_visited = 0;
	...
	/* pass back the above results */
	$return_array[1] = $set_goal;
	$return_array[2] = $cnum;
	$return_array[3] = $moves;
	$return_array[4] = $percent_visited;
	$return_array[5] = $random_browsing;
	$return_array[6] = $goal_last_visited;
	return $return_array;
}
