...
//	echo count($concept_time_map);
	if (count($concept_time_map) > 3) {
		$related_concept_topics = get_related_concept_topics(2);
	}
	...
	$concept_topic_to_be_suggested = array();
	for ($i=0; $i<$num_goal; $i++) {
		$con = $suggested_concepts[$i];
		if (!isset($concept_topic_to_be_suggested["" . $con . "_" . $goal_tid])) {
			$num_added++;
			$str .= get_anchor_text($current_uid, $goal_tid, $con, $num_added, false) . "<br/>";
			$concept_topic_to_be_suggested["" . $con . "_" . $goal_tid] = 1;
		}
	}

	if ($num_added == 0) {
		// TODO : hack for now: show only two related suggestions.
		$num_needed = 2;
	}
	...
	foreach($related_concept_topics as $ct => $tim) {
		...
		if (!isset($concept_topic_to_be_suggested["" . $con . "_" . $top])) {
			$num_added++;
			$str .= get_anchor_text($current_uid, $top, $con, $num_added, true) . "<br/>";
			$concept_topic_to_be_suggested["" . $con . "_" . $top] = 1;
		}
	}
